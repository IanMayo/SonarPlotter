...
Dygraph.Interaction.movePan = function(event, g, context) {
  ...
  if (context.is2DPan) {
    ...
    for (var i = 0; i < g.axes_.length; i++) {
      ...
      context.minValue = minValue; // LLL
      context.maxValue = maxValue; // LLL
  ...
  context.xrr = g.xAxisRange()[1];
  context.xrl = g.xAxisRange()[0];
...
Dygraph.Interaction.endPan = function(event, g, context) {
  ...
  if (context.xrr > 370) {g.updateOptions( { dateWindow : [xrls,360] } );}
  if (context.xrl < -10) {g.updateOptions( { dateWindow : [0,xrrs] } );}

//if ( > 360 || g.xAxisRange()[0] < 0) {g.updateOptions( { dateWindow : [0,360] } );}  
  g.updateOptions({valueRange: [context.minValue, context.maxValue]}); // LLL update valueRange after pan
